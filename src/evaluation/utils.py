...
def generate_prediction_list(input_sequences, tokenizer, model, num_recommendations, max_new_tokens=200, device='cpu'):
    ...
    predictions=[]
    scores=[]
    for preds_sequence,scores_sequence,sequence in zip(chunked(sample_output["sequences"].cpu(),num_recommendations),
                                                       chunked(sample_output["sequences_scores"].cpu(),num_recommendations),input_sequences):
    ...
    recommendations_with_score = [(r,round(float(s[0]),3)) for r,s in __ranking_max(predictions, scores,num_recommendations)]
